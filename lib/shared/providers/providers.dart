...
final digitStyleProvider =
    StateNotifierProvider<DigitStyleNotifier, bool>((ref) {
...
final summaryProvider = FutureProvider<SummaryStats>((ref) async {
  ...
  final pct = missedPrayers > 0
      ? (total / missedPrayers * 100).clamp(0.0, 100.0)
      : 0.0;
  ...
  final daysSinceStart = diffDays(plan.startDate, todayIso()).clamp(0, 99999);
...
final prayerProgressProvider =
    FutureProvider<List<PrayerProgressData>>((ref) async {
  final plan = await ref.watch(planProvider.future);
  final dao  = ref.watch(prayerLogDaoProvider);
...
final recentActivityProvider =
    FutureProvider<List<PrayerLogTableData>>((ref) async {
  final dao = ref.watch(prayerLogDaoProvider);
...
final selectedMonthProvider =
    StateProvider<String>((ref) => toYearMonth(DateTime.now()));
...
final calendarDataProvider =
    FutureProvider<Map<String, CalendarDayData>>((ref) async {
...
class CalendarTypeNotifier extends StateNotifier<CalendarType> {
  ...
  Future<void> toggle() async {
    final prefs   = await _ref.read(sharedPrefsProvider.future);
    final newType = state == CalendarType.miladi
        ? CalendarType.hijri
        : CalendarType.miladi;
    state = newType;
    await prefs.setString(
        _key, newType == CalendarType.hijri ? 'hijri' : 'miladi');
  }
}